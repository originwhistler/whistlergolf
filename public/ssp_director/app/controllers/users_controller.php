...
class UsersController extends AppController
{
	...
	var $non_ajax_actions = array('index', 'manage', '_list', 'profile', 'login', 'logout', 'password', 'clear_avatar', 'avatar_uri');
	...
	function avatar_uri($id, $w, $h, $x, $y) {
		...
		$t = filemtime($avs[0]);
		echo(p(basename($avs[0]), "avatar-$id", $w, $h, 0, 75, 1, $x, $y, false) . '&rand=' . $t);
		exit;
	}
	...
	function avatar_focal($id) {
		...
		$avs = glob(AVATARS . DS . $id . DS . 'original.*');
		$t = filemtime($avs[0]);
		die('<img src="' . p(basename($avs[0]), "avatar-$id", 100, 100, 1, 75, 1, $this->data['x'], $this->data['y'], false) . '&rand=' . $t . '" />');
	}
