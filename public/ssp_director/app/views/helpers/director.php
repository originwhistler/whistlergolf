...
class DirectorHelper extends AppHelper {
	...
	function albumBg($value, $path) {
		...
		if (empty($value)) {
			...
			$value = p($value, $path, 48, 37, 1, 70, 0, 50, 50);
		}
	...
	function getVidThumb($src, $path, $w = 84, $h = 63, $sq = 1, $q = 70, $sh = 1, $tail = false) {
		...
		if (!empty($custom)) {
			$filename = p(basename($custom[0]), $path, $w, $h, $sq, $q, $sh, 50, 50);
			if ($tail) { $filename .= '||v'; }
		} else if (isVideo($src)) {
	...
	function avatar($user_id, $w = 48, $h = 48, $anchor, $extra_styles = null) {
		...
		if (!empty($original)) {
			...
			$out .= p(basename($original[0]), "avatar-{$user_id}", $w, $h, 1, 75, 1, $x, $y, false) . '&amp;rand=' . $t;
		} else {
			$out .= $this->webroot . 'img/default_avatar.gif';
		}
	...
	function recentList($imgs, $max, $w, $h, $album = false) {
		...
		foreach($imgs as $img):
			...
			if (isNotImg($img['Image']['src'])):
				...
				$o .= p($img['Image']['src'], $img['Album']['path'], 250, 250, 0, 70, 1, $x, $y);
			endif;
			...
			if (isNotImg($img['Image']['src'])):
				$o .= $this->getVidThumb($img['Image']['src'], $img['Album']['path'], 84, 63, 1, 70, 1, true);
			 	...
			 	$o .= p($img['Image']['src'], $img['Album']['path'], 84, 63, 1, 70, 1, $x, $y);
			endif;
