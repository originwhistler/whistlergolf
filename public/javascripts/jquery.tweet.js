(function($) {
  ...
  $.fn.tweet = function(o){
    var s = {
      username: ["whistlergolf"],             // [string]   required, unless you want to display our tweets. :) it can be an array, just do ["username1","username2","etc"]
    ...
    return this.each(function(){
      ...
      $.getJSON(url, function(data){
        ...
        $.each(data.results, function(i,item){
          ...
          //var date = '<a target="_blank" href="http://twitter.com/'+item.from_user+'/statuses/'+item.id+'" title="view tweet on twitter">'+relative_time(item.created_at)+'</a>';
          var date = '<span class="twitter-time">' +relative_time(item.created_at) +"</span>";
          var text = '<span class="feed_description">' +$([item.text]).linkUrl().linkUser().linkHash().makeHeart().capAwesome().capEpic()[0]+ '</span>';
