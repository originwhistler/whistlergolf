...
var Accordion = Class.create({
    ...
    createTableHeights: function (){
    	for(var i=0; i<this.contents.length; i++) {
    		this.tableHeights.push(this.contents[i].getHeight());
        }
    },
    
    expand: function(el) {
        this.toExpand = el.next('div.'+this.options.contentClass);
        //alert (this.tableHeights[this.toExpand.id]);
        if(this.current != this.toExpand){
        	this.maxHeight= this.tableHeights[this.toExpand.id];
			this.toExpand.show();
            this.animate();
        }
    },
...
document.observe("dom:loaded", function(){
    accordion = new Accordion("programs-accordion", 6);
})*/